<template>
  ...
  >
    <div
      class="info-large"
      :class="{
        'info-button-glow': infoButtonGlow,
        'info-button': !store.infoScreen,
      }"
    >
      <div
      ...
      <div class="inner-grid text-black!" v-show="store.infoScreen">
        <div class="upper-content">
            ...
            <h1
              v-html="
                store.infoStepper.current[
                  ...
                  store.modesCycler.state.name === 'S2' ||
                  store.modesCycler.state.name === 'S3'
                    ? store.modesCycler.state.name
                    : 'Empty'
                ].title
              "
            ></h1>
          ...
          <div
            v-if="store.infoStepper.isCurrent('disclaimer')"
            class="flex flex-col items-center disclaimer"
          >
            <h1
              v-html="store.infoStepper.current.title"
              class="border-solid border-3px"
            ></h1>
            <p
              class="mt-10px! mb-40px!"
              v-html="store.infoStepper.current.upperText"
            ></p>
          </div>
          <div v-else>
            <h1
            ...
            ></h1>
            <img
              v-if="store.infoStepper.current.img"
              :src="store.infoStepper.current.img"
            />
            <p
              class="mt-10px! mb-40px!"
              v-html="store.infoStepper.current.subtitle"
            ></p>
          </div>
        </div>
        <div class="lower-content">
          <div
            v-if="store.infoStepper.isCurrent('welcome')"
          ...
          <div
            v-if="store.infoStepper.isCurrent('center-circle')"
            class="mt--10px!"
          >
            <!-- <DoubleTap />
                        <p class="mt--5px! mb-10px!">DOUBLE TAP to mute audio.</p> -->
            <Up class="mt--10px!" />
            <p class="mt--5px! mb-10px!">
              SWIPE UP to adopt the cochonnet’s point of view.
            </p>
            <p class="mb-10px!">
            ...
            <p class="mb--10px!">
              SWIPE DOWN to return to starting point perspective.
            </p>
            <Down />
          </div>
          <div
            v-if="store.infoStepper.isCurrent('functions')"
            class="mt--10px!"
          >
            <Dot class="mix-blend-difference" />
            <p class="mt--5px! mb-10px!">
              TAP once to run the function of the button. Depending on the
              function you receive feedback in the app or on the TV screens.
            </p>
            <LongPress class="mix-blend-difference" />
            <p class="mt--5px! mb-10px!">
              PRESS and HOLD to hear an explanation of the function. RELEASE to
              stop.
            </p>
          </div>
        </div>
    ...
    <div
      v-if="!store.infoStepper.isCurrent('welcome') && !smallHeight"
    ...
    >
      <div
        class="button-container info-back"
        :class="{ 'color-white!': !store.infoStepper.isCurrent('welcome') }"
        @click="store.infoStepper.goToPrevious()"
      >
        BACK
      </div>
      <div
        v-if="!smallHeight"
        class="button-container info-next"
        :class="{
          'color-white!':
            !store.infoStepper.isCurrent('welcome') &&
            !store.infoStepper.isLast,
          'start-button': store.infoStepper.isLast,
        }"
        @click="store.infoStepper.isLast ? close() : nextFunction()"
      >
        {{ store.infoStepper.isLast ? "START" : "NEXT" }}
      </div>
    </div>
    <div
      v-if="store.infoScreen"
...
const smallHeight = computed(() => height.value < 750);
...
.info-large {
  ...
  @apply: sm:max-w-80dvw sm:max-h-80dvw lg:max-w-800px lg:max-h-800px;
  ...
  padding: 2rem;
  z-index: 1000;
  box-shadow: 0px 4px 38.4px 22px #ffd3d3 inset,
