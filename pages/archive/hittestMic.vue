...
function init() {
  ...
  camera.add(listener); // Ensure listener is added to camera
...
function intersectPrediction(prediction) {
  ...
  if (intersects.length > 0) {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then((stream) => {
      ...
      sound.setRefDistance(0.5); // Reduced reference distance for more noticeable effect
      sound.setRolloffFactor(2); // Increased rolloff for more dramatic distance effect
      sound.setDistanceModel("inverse"); // Using inverse model for better spatialization
      sound.setMaxDistance(20); // Reduced max distance for more noticeable effect
      sound.setLoop(true);
      sound.setVolume(1);
      
      // Ensure the sound is properly positioned in 3D space
